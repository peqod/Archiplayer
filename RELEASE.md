...
- **CI owns the release and `SHA256SUMS.txt`.** The workflow builds Windows and attaches the checksums. You do not build or hash locally for a release.
- **Windows is the only distributed platform right now.** macOS and Linux are paused; both still build from source.
...
Release notes are not written on GitHub. Add the section to `CHANGELOG.md` before you release:
...
CI extracts that section and hands it to `tauri-action` as the release body. `release.ps1` runs
the same extract as a gate, so a missing or empty section aborts before the tag push. Check it
by hand with:

```powershell
node scripts/extract-changelog.mjs v0.4.0
```
...
This will, in order: check preconditions, bump every version file, run the version and changelog gates, commit `release: v0.4.0`, push `main`, **prompt for confirmation**, create and push the `v0.4.0` tag (starts CI), watch the run, verify both assets landed, publish the draft.
...
# 2. Gates: all versions must agree and match the tag, and the notes must exist
...
gh release view v0.4.0 --json assets --jq '.assets[].name'   # expect 2 assets
...
- **`publish` job:** a single-leg matrix (`fail-fast: false`), `windows-latest` (`nsis`). The matrix shape is kept so a paused platform is one block to restore. It runs the version gate, extracts the release notes from `CHANGELOG.md`, then runs `tauri-apps/tauri-action` with `releaseDraft: true`, creating/filling a **draft** release named `Archiplayer <tag>`.
- **`checksums` job (`needs: publish`):** downloads every asset, runs `sha256sum` into `SHA256SUMS.txt`, and uploads it. Because it *needs* `publish`, **if the build leg fails, checksums is skipped** and the draft is left partial. Do not publish it.

Expected assets on a complete release (2):
...
**Asset-name contract:** the website (`site/site.js`) matches assets by regex, Windows `/setup.*\.exe$|\.msi$/i` and checksums `/SHA256SUMS/i`. Do not rename these outputs. macOS and Linux are displayed as coming soon and have no asset matcher.

## Verify and publish

```powershell
gh release view v0.4.0 --json assets --jq '.assets[].name'   # confirm 2 assets
...
**Version-gate mismatch** means `verify-release-version.mjs` names the file that drifted (`package`, `tauri`, `cargo`, or `cargo-lock`). Fix that file to match and re-run the gate. `package-lock.json` is intentionally not gated.

**Changelog gate failed** means `CHANGELOG.md` has no section for the tag, or the section is empty. Add `## [X.Y.Z] - YYYY-MM-DD` with at least one bullet, then re-run. Nothing has been pushed at that point.

**The build leg failed** means the `publish` job failed, so `checksums` was skipped: the draft has partial assets and no `SHA256SUMS.txt`. Do **not** publish it. Fix the build, then re-tag (below).
