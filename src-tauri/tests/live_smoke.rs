...
async fn live_pipeline_catalog_show_playlist_audio() {
    ...
    let ep = eps
        .iter()
        .find(|e| e.archive_id.is_some())
        .expect("an ep with audio");
    eprintln!(
        "show WA: {} episodes; first playable ep {} arch {:?}",
        eps.len(),
        ep.id,
        ep.archive_id
    );
    ...
    // 3. Playlist → tracks
    let pl_html = f
        .get_text(&wfmu::playlist_url(ep.id))
        .await
        .expect("playlist fetch");
    ...
    let bt_html = f
        .get_text(&wfmu::show_url("BT"))
        .await
        .expect("BT show fetch");
    let bt_eps = wfmu::parse_show_page(&bt_html);
    let bt_playable = bt_eps.iter().filter(|e| e.archive_id.is_some()).count();
    eprintln!(
        "Brian Turner: {} episodes, {} playable",
        bt_eps.len(),
        bt_playable
    );
    assert!(
        bt_playable > 500,
        "expected most BT episodes playable, got {bt_playable}"
    );
    let old = bt_eps.last().expect("an old BT episode");
    let bt_player = f
        .get_text(&wfmu::archiveplayer_url(
            old.id,
            old.archive_id.expect("old BT archive"),
        ))
        .await
...
#[tokio::test]
#[ignore]
async fn every_live_station_exposes_history_and_schedule() {
    let fetcher = Fetcher::new();
    for id in ["freeform", "drummer", "rocknsoul", "sheena"] {
        let station = wfmu::live_station(id).expect("configured station");
        let body = fetcher
            .get_live_page_text(&wfmu::rethink_playlist_url(station.rethink_code))
            .await
            .expect("Radio Rethink snapshot");
        let page = wfmu::parse_rethink_live_page(&body).expect("snapshot parses");
        assert!(!page.tracks.is_empty(), "{id}: recent tracks");
        assert!(page.tracks.len() <= 20, "{id}: history is capped");
        assert!(page.current_show.is_some(), "{id}: current show");
        assert!(page.next_show.is_some(), "{id}: next show");

        let info = fetcher
            .get_live_page_text(station.info_url)
            .await
            .expect("WFMU station page");
        let schedule = wfmu::parse_live_schedule(&info);
        assert!(!schedule.is_empty(), "{id}: daily/weekly schedule");
    }
}
