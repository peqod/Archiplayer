<script lang="ts">
  // User-supplied brand icons, recolored via currentColor and sized to 1em.
  let {
    name,
    filled = false,
    size,
  }: {
    name:
      | "star"
      | "save"
      | "share"
      | "download"
      | "play"
      | "pause"
      | "playing"
      | "next"
      | "prev"
      | "next-ep"
      | "prev-ep"
      | "volume-loud"
      | "volume-quiet"
      | "volume-mute";
    filled?: boolean;
    size?: string;
  } = $props();

  // Default glyph runs ~35% larger than the cap height for better presence.
  // Follows a cascading --icon-size var so any ancestor can rescale icons; an explicit
  // `size` prop still wins per-instance.
  const box = $derived(size ?? "var(--icon-size, 1.35em)");
  // Each glyph also gets a per-name class (e.g. `icon-play`) so shared CSS can target one
  // glyph — used to optically-centre the play/playing triangles wherever they appear.
</script>

{#if name === "star"}
  <svg class="icon icon-{name}" style="width:{box};height:{box}" viewBox="0 0 24 24" fill={filled ? "currentColor" : "none"} xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <path
      d="M11.245 4.174C11.4765 3.50808 11.5922 3.17513 11.7634 3.08285C11.9115 3.00298 12.0898 3.00298 12.238 3.08285C12.4091 3.17513 12.5248 3.50808 12.7563 4.174L14.2866 8.57639C14.3525 8.76592 14.3854 8.86068 14.4448 8.93125C14.4972 8.99359 14.5641 9.04218 14.6396 9.07278C14.725 9.10743 14.8253 9.10947 15.0259 9.11356L19.6857 9.20852C20.3906 9.22288 20.743 9.23007 20.8837 9.36432C21.0054 9.48051 21.0605 9.65014 21.0303 9.81569C20.9955 10.007 20.7146 10.2199 20.1528 10.6459L16.4387 13.4616C16.2788 13.5829 16.1989 13.6435 16.1501 13.7217C16.107 13.7909 16.0815 13.8695 16.0757 13.9507C16.0692 14.0427 16.0982 14.1387 16.1563 14.3308L17.506 18.7919C17.7101 19.4667 17.8122 19.8041 17.728 19.9793C17.6551 20.131 17.5108 20.2358 17.344 20.2583C17.1513 20.2842 16.862 20.0829 16.2833 19.6802L12.4576 17.0181C12.2929 16.9035 12.2106 16.8462 12.1211 16.8239C12.042 16.8043 11.9593 16.8043 11.8803 16.8239C11.7908 16.8462 11.7084 16.9035 11.5437 17.0181L7.71805 19.6802C7.13937 20.0829 6.85003 20.2842 6.65733 20.2583C6.49056 20.2358 6.34626 20.131 6.27337 19.9793C6.18915 19.8041 6.29123 19.4667 6.49538 18.7919L7.84503 14.3308C7.90313 14.1387 7.93218 14.0427 7.92564 13.9507C7.91986 13.8695 7.89432 13.7909 7.85123 13.7217C7.80246 13.6435 7.72251 13.5829 7.56262 13.4616L3.84858 10.6459C3.28678 10.2199 3.00588 10.007 2.97101 9.81569C2.94082 9.65014 2.99594 9.48051 3.11767 9.36432C3.25831 9.23007 3.61074 9.22289 4.31559 9.20852L8.9754 9.11356C9.176 9.10947 9.27631 9.10743 9.36177 9.07278C9.43726 9.04218 9.50414 8.99359 9.55657 8.93125C9.61593 8.86068 9.64887 8.76592 9.71475 8.57639L11.245 4.174Z"
      stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" />
  </svg>
{:else if name === "save"}
  <svg class="icon icon-{name}" style="width:{box};height:{box}" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    {#if filled}
      <path d="M6.75 6L7.5 5.25H16.5L17.25 6V19.3162L12 16.2051L6.75 19.3162V6Z" fill="currentColor" />
    {:else}
      <path fill-rule="evenodd" clip-rule="evenodd" d="M6.75 6L7.5 5.25H16.5L17.25 6V19.3162L12 16.2051L6.75 19.3162V6ZM8.25 6.75V16.6838L12 14.4615L15.75 16.6838V6.75H8.25Z" fill="currentColor" />
    {/if}
  </svg>
{:else if name === "share"}
  <svg class="icon icon-{name}" style="width:{box};height:{box}" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
    <circle cx="18" cy="5" r="2.75" />
    <circle cx="6" cy="12" r="2.75" />
    <circle cx="18" cy="19" r="2.75" />
    <path d="M8.4 13.35L15.6 17.65M15.6 6.35L8.4 10.65" />
  </svg>
{:else if name === "download"}
  <svg class="icon icon-{name}" style="width:{box};height:{box}" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <path fill-rule="evenodd" clip-rule="evenodd" d="M12 15.9853L15.182 12.8033L14.1213 11.7427L12.75 13.114L12.75 5.25L11.25 5.25L11.25 13.114L9.8787 11.7427L8.81804 12.8033L12 15.9853ZM12 13.864L12 13.864L12.0001 13.864L12 13.864Z" fill="currentColor" />
    <path d="M18 17.25L18 18.75L6 18.75L6 17.25L18 17.25Z" fill="currentColor" />
  </svg>
{:else if name === "play"}
  <svg class="icon icon-{name}" style="width:{box};height:{box}" viewBox="0 0 500 501" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <g transform="matrix(1,0,0,1,-2597.157561,-1197.938257)"><g transform="matrix(1,0,0,1,680.600271,0)">
      <path d="M1916.557,1698.134L1916.557,1197.938L2413.805,1448.036L1916.557,1698.134ZM1955.724,1634.593L2326.638,1448.036L1955.724,1261.479L1955.724,1634.593Z" />
    </g></g>
  </svg>
{:else if name === "pause"}
  <svg class="icon icon-{name}" style="width:{box};height:{box}" viewBox="0 0 498 501" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <g transform="matrix(1,0,0,1,-1916.557289,-1197.938257)"><g transform="matrix(0.359859,0,0,1,1495.577341,0)">
      <path d="M1670.043,1197.938L1670.043,1698.134L1169.847,1698.134L1169.847,1197.938L1670.043,1197.938ZM2551.63,1197.938L2551.63,1698.134L2051.435,1698.134L2051.435,1197.938L2551.63,1197.938Z" />
    </g></g>
  </svg>
{:else if name === "playing"}
  <svg class="icon icon-{name}" style="width:{box};height:{box}" viewBox="0 0 500 501" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <g transform="matrix(1,0,0,1,-2597.157561,-654.733829)"><g transform="matrix(1,0,0,1,680.600271,-543.204428)">
      <path d="M1916.557,1698.134L2413.805,1448.036L1916.557,1197.938L1916.557,1698.134Z" />
    </g></g>
  </svg>
{:else if name === "next"}
  <svg class="icon icon-{name}" style="width:{box};height:{box}" viewBox="0 0 511 324" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <g transform="matrix(1,0,0,1,-1916.557289,-751.886127)"><g transform="matrix(0.855133,0,0,1,277.646458,-543.204428)">
      <path d="M1916.557,1619.089L1916.557,1295.091L2253.942,1448.036L1916.557,1619.089ZM2176.423,1523.447L2325.164,1448.036L2176.423,1380.608L2176.423,1295.091L2513.808,1448.036L2176.423,1619.089L2176.423,1523.447Z" />
    </g></g>
  </svg>
{:else if name === "prev"}
  <svg class="icon icon-{name}" style="width:{box};height:{box};transform:scaleX(-1)" viewBox="0 0 511 324" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <g transform="matrix(1,0,0,1,-1916.557289,-751.886127)"><g transform="matrix(0.855133,0,0,1,277.646458,-543.204428)">
      <path d="M1916.557,1619.089L1916.557,1295.091L2253.942,1448.036L1916.557,1619.089ZM2176.423,1523.447L2325.164,1448.036L2176.423,1380.608L2176.423,1295.091L2513.808,1448.036L2176.423,1619.089L2176.423,1523.447Z" />
    </g></g>
  </svg>
{:else if name === "next-ep"}
  <svg class="icon icon-{name}" style="width:{box};height:{box}" viewBox="0 0 390 324" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <g transform="matrix(1,0,0,1,-1916.557289,-340.237111)"><g transform="matrix(0,1,-1,0,2992.441462,-1576.320178)">
      <path d="M2084.576,763.926L2240.555,1075.884L1916.557,1075.884L2072.536,763.926L1916.557,763.926L1916.557,686.554L2240.555,686.554L2240.555,763.926L2084.576,763.926Z" />
    </g></g>
  </svg>
{:else if name === "prev-ep"}
  <svg class="icon icon-{name}" style="width:{box};height:{box};transform:scaleX(-1)" viewBox="0 0 390 324" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <g transform="matrix(1,0,0,1,-1916.557289,-340.237111)"><g transform="matrix(0,1,-1,0,2992.441462,-1576.320178)">
      <path d="M2084.576,763.926L2240.555,1075.884L1916.557,1075.884L2072.536,763.926L1916.557,763.926L1916.557,686.554L2240.555,686.554L2240.555,763.926L2084.576,763.926Z" />
    </g></g>
  </svg>
{:else}
  <svg class="icon icon-{name}" style="width:{box};height:{box}" viewBox="0 0 512 512" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <g fill="currentColor" transform="translate(42.666667, 85.333333)">
      {#if name === "volume-loud"}
        <path d="M361.299413,341.610667 L328.014293,314.98176 C402.206933,233.906133 402.206933,109.96608 328.013013,28.8906667 L361.298133,2.26304 C447.910187,98.97536 447.908907,244.898347 361.299413,341.610667 Z M276.912853,69.77216 L243.588693,96.4309333 C283.38432,138.998613 283.38304,204.87488 243.589973,247.44256 L276.914133,274.101333 C329.118507,215.880107 329.118507,127.992107 276.912853,69.77216 Z M191.749973,0 L80.8957867,87.2292267 L0,87.2292267 L0,257.895893 L81.0208,257.895893 L191.749973,343.35424 L191.749973,0 Z M42.6666667,129.895893 L95.6874667,129.895893 L149.083307,87.8749867 L149.083307,256.520747 L95.5624533,215.229227 L42.6666667,215.229227 L42.6666667,129.895893 Z" />
      {:else if name === "volume-quiet"}
        <path d="M276.914133,274.101547 L243.589973,247.442773 C283.38304,204.875093 283.38432,138.998827 243.588693,96.4311467 L276.912853,69.77216 C329.118507,127.992107 329.118507,215.880107 276.914133,274.101547 Z M191.749973,0 L80.8957867,87.2292267 L0,87.2292267 L0,257.895893 L81.0208,257.895893 L191.749973,343.35424 L191.749973,0 Z M42.6666667,129.895893 L95.6874667,129.895893 L149.083307,87.8749867 L149.083307,256.520747 L95.5624533,215.229227 L42.6666667,215.229227 L42.6666667,129.895893 Z" />
      {:else}
        <path d="M360.708267,171.937493 L420.291627,231.520853 L390.125013,261.687467 L330.541653,202.104107 L270.958293,261.687467 L240.79168,231.520853 L300.37504,171.937493 L240.79168,112.354133 L270.958293,82.18752 L330.541653,141.77088 L390.125013,82.18752 L420.291627,112.354133 L360.708267,171.937493 Z M191.749973,0 L80.8957867,87.2292267 L0,87.2292267 L0,257.895893 L81.0208,257.895893 L191.749973,343.35424 L191.749973,0 Z M42.6666667,129.895893 L95.6874667,129.895893 L149.083307,87.8749867 L149.083307,256.520747 L95.5624533,215.229227 L42.6666667,215.229227 L42.6666667,129.895893 Z" />
      {/if}
    </g>
  </svg>
{/if}

<style>
  .icon {
    display: inline-block;
    vertical-align: -0.15em;
    flex: 0 0 auto;
  }
</style>
